...
function songsWithBodySuits() {
  return beyonceHash.hits.filter(hit => hits.signature_look.includes('bodysuit') || hit.video_theme.includes('bodysuit'))
};
...
function getSongsByFiercenessGTE() {
  return beyonceHash.hits.filter(hit => hit.fierceness >= 15)
};
...
function groupHits() {
  return beyonceHash.hits.filter(song => song.group === true)
};

// 9. Return a hit song where Beyonce's hair is blonde
function findBlondeHit() {
  return beyonceHash.hits.filter(song => song.hair.includes('blonde'))
};

// 10. Return the hit song "Sorry"
function sorry() {
  return beyonceHash.hits.find(song => song.title === 'sorry')
};

// 11. Return a given song
function getSong() {
  return beyonceHash.hits.find(song => song.title)
}
...
function uniqueHairstyles() {
  return [...new Set(beyonceHash.hits
    .map(pello => pello.hair)
